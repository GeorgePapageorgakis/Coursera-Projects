/**
    MP3
    Cuda_Tiled_Matrix_Mul.cpp
    Purpose: Implement a tiled dense matrix multiplication routine using shared memory CUDA API.
...
__global__ void matrixMultiplyShared(float *A, float *B, float *C, int numARows,
	...
	for (int t = 0; t < numAColumns/TILE_WIDTH + 1; t++) {
...
int main(int argc, char **argv) {
  ...
  wbCheck( cudaMemcpy(deviceB, hostB, numBRows * numCColumns * sizeof(float), cudaMemcpyHostToDevice)); 
  wbTime_stop(GPU, "Copying input memory to the GPU.");
	
  //@@ Initialize the grid and block dimensions
  dim3 blockDim(TILE_WIDTH, TILE_WIDTH, 1);
  dim3 gridDim(ceil(numCColumns-1/blockDim.x), ceil(numCRows-1/blockDim.y), 1);
